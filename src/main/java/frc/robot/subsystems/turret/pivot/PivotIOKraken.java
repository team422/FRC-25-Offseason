...
public class PivotIOKraken implements PivotIO {
  ...
  public PivotIOKraken(int port, String bus) {
    ...
    var currentLimits =
        new CurrentLimitsConfigs()
            ...
            .withSupplyCurrentLimit(CurrentLimitConstants.kPivotDefaultSupplyCurrentLimit)
            .withStatorCurrentLimitEnable(true)
            .withStatorCurrentLimit(CurrentLimitConstants.kPivotDefaultStatorCurrentLimit);

    var motorOutput = new MotorOutputConfigs().withNeutralMode(NeutralModeValue.Coast);

    var feedback = new FeedbackConfigs().withSensorToMechanismRatio(PivotConstants.kGearRatio);
  ...
  public void setPID(int slot, double p, double i, double d) {
    var config = new SlotConfigs().withKP(p).withKI(i).withKD(d);
    config.SlotNumber = 0;
